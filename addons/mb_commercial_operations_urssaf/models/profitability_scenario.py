...
class MbCommercialProfitabilityScenarioLine(models.Model):
    ...
    urssaf_cotisation_rate_id = fields.Many2one(
        "l10n.fr.micro.urssaf.rate", ondelete="restrict", readonly=True,
    )
    ...
    def _urssaf_planning_values(self, company, date):
        ...
        chamber = rate_model.rate_for("chamber", "bic_goods", date, company) \
            if company.l10n_fr_micro_chamber_kind != "none" else rate_model
        liberatoire = rate_model.rate_for("liberatoire", "bic_goods", date, company) \
            if company.l10n_fr_micro_versement_from \
            and company.l10n_fr_micro_versement_from <= date \
            and (not company.l10n_fr_micro_versement_to or date <= company.l10n_fr_micro_versement_to) \
            else rate_model
        ...
        if self.apply_urssaf_goods_rates:
            ...
            values.update(
                self.env["mb.commercial.profitability.scenario.line"]._urssaf_planning_values(
                    self.wizard_id.company_id, date,
                )
            )
        return values
