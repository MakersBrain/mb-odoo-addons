...
class MbMarketStockPlanLine(models.Model):
        ...
        for line in self.filtered(lambda item: item.supply_method == "purchase"):
            ...
            if not product:
                raise ValidationError(
                    _("Map the assortment shortage to a concrete purchasable product first.")
                )
            ...
            if quantity <= 0:
                ...
                continue
            active = line.purchase_line_ids.filtered(
            ...
            if not vendor:
                raise ValidationError(
                    _("Choose a vendor for %(product)s.", product=product.display_name)
                )
            operation = line.operation_id
            order = operation.purchase_order_ids.filtered(
                lambda purchase, selected_vendor=vendor: (
                    purchase.partner_id == selected_vendor and purchase.state in ("draft", "sent")
                )
            )[:1]
            if not order:
            ...
            seller = product._select_seller(
                ...
                date=fields.Date.to_date(
                    operation.stock_preparation_deadline or operation.planned_start
                ),
    ...
    def _update_supply_readiness(self):
        ...
        for line in self.filtered(lambda item: item.supply_method == "purchase"):
            purchase_lines = line.purchase_line_ids.filtered(
                lambda item: item.order_id.state != "cancel"
            )
            state = "shortage"
            ...
            if purchase_lines:
                ...
                if any(order.state in ("draft", "sent", "to approve") for order in orders):
                    state, note = (
                        "supply_proposed",
                        _("Draft RFQ supply awaits review and confirmation."),
                    )
                elif all(order.state == "done" for order in orders):
                    state, note = (
                        "ready_to_pick",
                        _("Purchasing is complete; refresh stock availability."),
                    )
                else:
                    cutoff = (
                    ...
                    at_risk = any(
                        item.date_planned and item.date_planned > cutoff for item in purchase_lines
                    )
                    started = any(
                        order.picking_ids.filtered(
                            lambda picking: picking.state not in ("draft", "cancel", "done")
                        )
                        for order in orders
                    )
                    state = (
                        "at_risk" if at_risk else "in_progress" if started else "supply_confirmed"
                    )
                    note = (
                        _("Purchased supply is scheduled after the preparation cutoff.")
                        if at_risk
                        else _("Purchase supply is confirmed.")
                    )
