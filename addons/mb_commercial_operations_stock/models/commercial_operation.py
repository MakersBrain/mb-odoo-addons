...
class MbCommercialOperation(models.Model):
    ...
    source_warehouse_id = fields.Many2one(
        "stock.warehouse", check_company=True, tracking=True,
    )
    ...
    preparation_picking_id = fields.Many2one(
        "stock.picking", check_company=True, copy=False, ondelete="restrict",
    )
    return_picking_ids = fields.One2many(
        "stock.picking", "mb_commercial_operation_id",
    ...
    def write(self, vals):
        allocation_fields = {
            "source_warehouse_id", "source_location_id", "market_location_id",
    ...
    def _compute_stock_status(self):
        for operation in self:
            ...
            operation.stock_reconciled = bool(operation.preparation_picking_id) and not discrepancies
    ...
    def _stock_discrepancies(self):
        ...
        sold = self._move_line_buckets(outbound_lines.filtered(
            lambda line: line.location_dest_id.usage == "customer"
        ))
        scrapped = self._move_line_buckets(outbound_lines.filtered(
            lambda line: line.move_id.scrap_id
        ))
        ...
        for product_id, lot_id in sorted(keys):
            difference = prepared[(product_id, lot_id)] - (
                returned[(product_id, lot_id)] + sold[(product_id, lot_id)]
                + scrapped[(product_id, lot_id)] + remaining[(product_id, lot_id)]
            )
        ...
        for line in self.stock_plan_line_ids.sorted("priority"):
            ...
            if line.target_type == "product":
                ...
                products_and_quantities = (
                    [(line.product_id, line.required_qty)] if not allocations
                    else [(allocation.product_id, allocation.quantity) for allocation in allocations]
                )
            else:
                products_and_quantities = [
            ...
            for product, quantity in products_and_quantities:
                ...
                matching = allocations.filtered(lambda allocation, p=product: allocation.product_id == p)
                if matching:
    ...
    def action_prepare_market_return(self):
        ...
        existing = self.return_picking_ids.filtered(lambda picking: picking.state not in ("done", "cancel"))[:1]
        if existing:
    ...
    def action_reopen_stock(self):
        if not self.env.user.has_group(
            ...
            raise UserError(_("Only a Commercial Operations Manager can reopen stock reconciliation."))
...
class MbMarketStockPlanLine(models.Model):
    ...
    supply_method = fields.Selection(selection_add=[("stock", "Take From Stock")], ondelete={"stock": "set default"})
    source_warehouse_id = fields.Many2one(
        "stock.warehouse", check_company=True,
        related="operation_id.source_warehouse_id", readonly=True,
    )
    source_location_id = fields.Many2one(
        "stock.location", check_company=True,
        related="operation_id.source_location_id", readonly=True,
    )
    allocation_ids = fields.One2many(
        "mb.market.stock.allocation", "plan_line_id", string="Concrete Stock",
    )
    ...
    def _refresh_availability(self):
        for line in self:
            ...
            if not warehouse:
                raise ValidationError(_("Choose a source warehouse on %(operation)s.", operation=operation.display_name))
            ...
            incoming = sum(
                ...
                if move.location_dest_id._child_of(location) and not move.location_id._child_of(location)
            )
            outgoing = sum(
                move.product_uom._compute_quantity(move.product_uom_qty, line.product_id.uom_id)
                for move in candidates
                if move.location_id._child_of(location) and not move.location_dest_id._child_of(location)
            )
            ...
            line.with_context(mb_stock_refresh=True).write({
                ...
                "blocking_note": False if forecast >= line.required_qty else _("Stock shortage: %(quantity)s", quantity=max(0.0, line.required_qty - forecast)),
            })
    ...
    def write(self, vals):
        refresh_fields = {
            "on_hand_now", "reserved_now", "incoming_before_cutoff",
            "outgoing_before_cutoff", "forecast_available", "shortage_qty",
            "availability_calculated_at", "readiness", "blocking_note",
        }
...
class MbMarketStockAllocation(models.Model):
    ...
    plan_line_id = fields.Many2one(
        "mb.market.stock.plan.line", required=True, check_company=True,
    ...
    lot_id = fields.Many2one(
        "stock.lot", check_company=True,
        domain="[('product_id', '=', product_id)]",
    )
    quantity = fields.Float(required=True, default=1.0)

    _positive_quantity = models.Constraint(
        "CHECK(quantity > 0)", "Allocated quantity must be positive.",
    )
    ...
    def _check_allocation(self):
        for allocation in self:
            ...
            if line.target_type == "product" and line.product_id != allocation.product_id:
                raise ValidationError(_("The allocated product must match the exact-product target."))
