...
class ResCompany(models.Model):
    ...
    @api.constrains(
        "mb_inventory_ai_enabled",
        "mb_inventory_vision_primary",
        ...
        for company in self:
            if company.mb_inventory_ai_enabled and not company.mb_inventory_vision_primary:
                raise ValidationError(
                    _("Choose an inventory vision provider before enabling AI fallback.")
                )
            ...
            if (
                company.mb_inventory_vision_secondary == company.mb_inventory_vision_primary
