...
class TestInventoryCapture(TransactionCase):
    ...
    def test_reviewed_rotated_crop_is_traceable_and_preferred_for_extraction(self):
        ...
        crop = capture.create_lot_crop(
            source["id"], [0.1, 0.2, 0.9, 0.8], 90, enhance=True,
        )
    ...
    def test_scan_frame_does_not_silently_choose_between_products(self):
        other = self.env["product.product"].create({
            "name": "Other glaze", "is_storable": True, "barcode": "4006381333931",
        })
        capture = self.capture()
    ...
    def test_external_lookup_keeps_normalized_review_candidate_only(self):
        ...
        candidate = capture.candidate_ids.filtered(lambda item: item.source == "upcitemdb_exact_gtin")
    ...
    def test_manager_created_online_product_keeps_verified_exact_gtin(self):
        ...
        response = {
            ...
            "gtin14": "04006381333931", "cache": "miss",
            "candidates": [{
    ...
    def test_review_candidate_can_be_mapped_to_an_existing_product(self):
        ...
        capture.ingest_result({
            ...
            "kind": "multimodal", "provider": "fixture-ai", "model": "fixture-v1",
            "state": "succeeded", "input_digests": [asset["content_sha256"]],
            "normalized_response": {"candidates": [{
                "kind": "product", "raw_value": "Possible Mayco glaze",
                "confidence": 0.6, "grounding_state": "unverified",
            }]},
        })
