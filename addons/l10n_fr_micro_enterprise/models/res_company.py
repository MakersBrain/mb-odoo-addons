...
class ResCompany(models.Model):
	...
	l10n_fr_micro_edi_mode = fields.Selection(
		...
		string="Electronic invoicing route", default="external_facturx",
		required=True, copy=False,
	)
	l10n_fr_micro_goods_tax_id = fields.Many2one(
		"account.tax", string="Franchise tax — goods", readonly=True, copy=False,
		check_company=True,
	)
	l10n_fr_micro_tax_group_id = fields.Many2one(
		"account.tax.group", string="Franchise tax group", readonly=True, copy=False,
		check_company=True,
	)
	...
	l10n_fr_micro_tax_switch_date = fields.Date(
		string="Current VAT regime effective from", readonly=True, copy=False,
	)
	l10n_fr_micro_tax_switch_user_id = fields.Many2one(
		"res.users", string="Switched by", readonly=True, copy=False,
	)
	...
	def _l10n_fr_micro_check_manager(self):
		if not self.env.is_superuser() and not self.env.user.has_group("account.group_account_manager"):
			raise AccessError(_("Only an Accounting Administrator can change the micro-enterprise VAT regime."))
	...
	def _l10n_fr_micro_prepare_tax_group(self, country):
		self.ensure_one()
		group_model = self.env["account.tax.group"].with_company(self).with_context(active_test=False)
	...
	def _l10n_fr_micro_switch(self, regime, effective_date=None):
		...
		for company in self:
			switch_date = fields.Date.to_date(effective_date) if effective_date else fields.Date.context_today(company)
	...
	def action_l10n_fr_micro_prepare_tax_setup(self):
		...
		return {"type": "ir.actions.client", "tag": "display_notification", "params": {
			...
			"message": _("The franchise taxes and domestic fiscal position are ready. No regime was changed."),
	...
	def action_l10n_fr_micro_activate_franchise(self):
		...
		return {"type": "ir.actions.client", "tag": "display_notification", "params": {
			...
			"message": _("Future domestic sales will use the Article 293 B exemption. Existing documents were not changed."),
