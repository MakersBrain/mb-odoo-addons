...
class L10nFrMicroSetupWizard(models.TransientModel):
    ...
    bank_account_id = fields.Many2one(
        "res.partner.bank",
        string="Professional bank account",
        ...
        values.update(
            {
                ...
                "tax_effective_date": company.l10n_fr_micro_tax_switch_date
    ...
    def _compute_readiness(self):
        for wizard in self:
            ...
            checks = [
                ...
                (
                    wizard.street and wizard.zip and wizard.city and wizard.country_id,
            ...
            if missing:
                ...
                wizard.readiness_html = Markup(
                    "<div class='alert alert-warning mb-0'><strong>%s</strong><ul class='mb-0'>%s</ul></div>"
                ) % (
                    _("Factur-X setup is incomplete:"),
                    items,
                )
            else:
                wizard.readiness_html = Markup(
                    "<div class='alert alert-success mb-0'><strong>%s</strong></div>"
                ) % _("Ready. Click Save and validate below to apply this setup to the company.")
    ...
    def action_apply(self):
        ...
        if self.tax_regime in ("franchise", "vat"):
            company._l10n_fr_micro_switch(
                self.tax_regime,
                effective_date=self.tax_effective_date,
            )
        else:
