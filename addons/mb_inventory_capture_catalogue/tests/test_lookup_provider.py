...
class TestInventoryCaptureCatalogueLookup(TransactionCase):
    ...
    def test_exact_barcode_result_is_grounded_without_creating_product(self):
        ...
        candidate = capture.candidate_ids.filtered(lambda item: item.source == "mb_catalogue")
    ...
    def test_manager_can_import_and_select_catalogue_candidate(self):
        ...
        template = self.env["product.template"].create(
            {
                "name": "Imported Hot Tamale",
                "is_storable": True,
            }
        )
        service = self.env["mb.catalogue.service"].search([], limit=1)
        with (
            patch.object(type(service), "action_import", autospec=True) as imported,
            patch.object(
                type(self.env["product.template"]),
                "_mb_find_by_canonical",
                autospec=True,
