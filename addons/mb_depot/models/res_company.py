    ...
    def write(self, values):
        if "mb_depot_stock_closed_through" in values:
            if not self.env.is_superuser() and not self.env.user.has_group(
                ...
                raise AccessError(
                    _("Only an Accounting Administrator can close depot inventory history.")
                )
            new_date = (
                fields.Date.to_date(values["mb_depot_stock_closed_through"])
                if values["mb_depot_stock_closed_through"]
                else False
            )
            for company in self:
                if company.mb_depot_stock_closed_through and (
                    ...
                    raise ValidationError(
                        _("The depot inventory closing horizon cannot be reduced.")
                    )
