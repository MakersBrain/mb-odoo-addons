...
class TestDepotSale(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.at_home = cls.env["product.product"].create(
            {
                "name": "Bowl still at the atelier",
                "type": "consu",
                "is_storable": True,
                "list_price": 100.0,
            }
    ...
    def _wizard(self, commission=40.0, **values):
        return self.env["mb.depot.create"].create(
            dict(
                {
    ...
    def test_a_depot_is_a_warehouse_with_its_own_stock(self):
        ...
        self.assertNotEqual(depot, self.warehouse, "the gallery's shelf is not the atelier's")
    ...
    def test_a_confirmed_depot_sale_ships_from_the_gallery(self):
        ...
        order = self.env["sale.order"].create(
            {
                "partner_id": self.gallery.id,
                "order_line": [
                    fields.Command.create({"product_id": self.held.id, "product_uom_qty": 1})
                ],
            }
        )
        order.action_confirm()
        ...
        self.assertEqual(
            picking.move_ids.quantity, 1, "and it reserves, because the stock is really there"
        )
    ...
    def test_new_products_invoice_on_delivered_quantities(self):
        ...
        self.assertEqual(
            self.env["ir.default"]._get("product.template", "invoice_policy"), "delivery"
        )
    ...
    def test_a_depot_sale_cannot_be_invoiced_before_it_ships(self):
        ...
        order.action_confirm()
        self.assertEqual(
            order.invoice_status, "no", "nothing to invoice until the piece leaves the depot"
        )
    ...
    def test_existing_storable_sale_products_move_to_delivery_policy(self):
        ...
        self.assertEqual(
            service.invoice_policy, "order", "services do not depend on a stock delivery"
        )
    ...
    def test_an_order_to_a_depositary_offers_only_what_it_holds(self):
        ...
        depot = self._depot()
        self._place(self.held, 2, depot.lot_stock_id)
        self.env["stock.quant"]._update_available_quantity(self.at_home, self.stock, 5)

        order = self.env["sale.order"].create({"partner_id": self.gallery.id})

        self.assertEqual(order.warehouse_id, depot, "the depot follows from the customer")
        self.assertIn(self.held, order.mb_depot_product_ids)
        self.assertNotIn(
            self.at_home,
            order.mb_depot_product_ids,
    ...
    def test_an_ordinary_customer_restricts_nothing(self):
        ...
        self.assertFalse(
            order.mb_depot_product_ids, "with no depot the line domain adds no clause at all"
        )
    ...
    def test_availability_is_read_at_the_depot(self):
        ...
        self.assertEqual(line.qty_available_today, 2, "the piece is on the shelf being sold from")
    ...
    def test_availability_of_an_ordinary_order_is_untouched(self):
        ...
        self.env["stock.quant"]._update_available_quantity(self.at_home, self.stock, 7)
        ...
        order = self.env["sale.order"].create(
            {
                ...
                "order_line": [
                    fields.Command.create({"product_id": self.at_home.id, "product_uom_qty": 1})
                ],
            }
        )
        ...
        self.assertFalse(order.warehouse_id.is_depot)
        self.assertEqual(
            order.order_line.qty_available_today,
            7,
