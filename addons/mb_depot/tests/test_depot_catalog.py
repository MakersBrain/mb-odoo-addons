...
class TestDepotCatalog(TransactionCase):
    ...
    def test_the_placement_is_recognised_as_one(self):
        self.assertTrue(
            self._placement().is_depot_placement, "the Catalogue button hangs off this flag"
        )

    def test_depot_stock_has_commercial_values_separate_from_cost(self):
        self.env["stock.quant"]._update_available_quantity(self.bowl, self.depot, 2)
    ...
    def test_the_catalog_reports_what_is_already_on_the_transfer(self):
        ...
        info = picking._get_product_catalog_order_line_info([self.bowl.id], child_field="move_ids")
    ...
    def test_only_storable_products_are_offered(self):
        ...
        products = self.env["product.product"].search(picking._get_product_catalog_domain())

        self.assertIn(self.bowl, products)
        self.assertNotIn(self.service, products, "a service cannot stand on a gallery shelf")
