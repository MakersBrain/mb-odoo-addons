...
class TestDepotSaleReport(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.gallery = cls.env["res.partner"].create({
            "name": "Depot report gallery", "is_company": True,
        })
        ...
        cls.product = cls.env["product.product"].create({
            "name": "Reported bowl", "type": "consu", "is_storable": True,
            "sale_ok": True, "invoice_policy": "delivery", "list_price": 120.0,
        })
    ...
    def test_optional_invoice_uses_today_and_delivery_period(self):
        ...
        ], reference="DEPOT-AUG-INVOICE", invoice=True)
    ...
    def test_serial_numbers_are_reserved_exactly(self):
        ...
        serial_product = self.env["product.product"].create({
            "name": "Unique reported vase", "type": "consu", "is_storable": True,
            "sale_ok": True, "invoice_policy": "delivery", "tracking": "serial",
            "list_price": 150.0,
        })
        first = self.env["stock.lot"].create({
            "name": "VASE-001", "product_id": serial_product.id,
        })
        second = self.env["stock.lot"].create({
            "name": "VASE-002", "product_id": serial_product.id,
        })
    ...
    def test_stock_in_depot_sub_location_uses_the_exact_quant_location(self):
        ...
        relocation.move_line_ids = [fields.Command.create({
            "product_id": self.product.id,
            "location_id": self.depot.lot_stock_id.id,
            "location_dest_id": depot_bin.id,
            "quantity": 1.0,
    ...
    def test_serial_that_previously_left_the_depot_is_rejected(self):
        now = fields.Datetime.now()
        serial_product = self.env["product.product"].create({
            "name": "Returned serial vase", "type": "consu", "is_storable": True,
            "sale_ok": True, "invoice_policy": "delivery", "tracking": "serial",
        })
        serial = self.env["stock.lot"].create({
            "name": "VASE-RETURNED", "product_id": serial_product.id,
        })
    ...
    def test_mandate_depot_is_rejected_before_document_creation(self):
        gallery = self.env["res.partner"].create({
            "name": "Mandate report gallery", "is_company": True,
        })
        ...
        mandate = self.env["stock.warehouse"].search([
    ...
    def test_downstream_failure_rolls_back_generated_documents(self):
        ...
        with self.assertRaisesRegex(UserError, "forced validation failure"), \
                self.env.cr.savepoint(), patch.object(
    ...
    def test_depot_sale_manager_can_process_without_accounting_access(self):
        ...
        manager = self.env["res.users"].create({
            ...
            "group_ids": [fields.Command.set(
                self.env.ref("mb_depot.group_depot_sale_manager").ids
            )],
        })
        report = self._report([
            {"sold_at": now - timedelta(days=2)},
    ...
    def test_completed_standard_return_marks_report_reversed(self):
        ...
        result = return_picking.with_context(
            skip_backorder=True, cancel_backorder=True,
        ).button_validate()
