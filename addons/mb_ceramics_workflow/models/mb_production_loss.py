...
class MbProductionLoss(models.Model):
    ...
    operation_id = fields.Many2one(
        "mrp.workorder", string="Operation", ondelete="restrict", check_company=True
    )
    ...
    board_id = fields.Many2one("stock.package", ondelete="restrict", check_company=True)
    firing_id = fields.Many2one("mb.firing", ondelete="restrict", check_company=True)
        ...
        for loss in self:
            if loss.operation_id and loss.operation_id.production_id != loss.production_id:
                raise ValidationError(
                    _("The loss operation must belong to its manufacturing order.")
                )
