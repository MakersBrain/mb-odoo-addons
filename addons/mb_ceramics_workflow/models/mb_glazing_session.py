    ...
    def create(self, vals_list):
        for values in vals_list:
            if values.get("name", _("New")) == _("New"):
                values["name"] = self.env["ir.sequence"].next_by_code("mb.glazing.session") or _(
                    "New"
                )
        ...
        for session in self:
            ...
            if any(
                ...
                raise ValidationError(
                    _("The board, locations and glazing session must share a company.")
                )
...
class MbGlazingSessionLine(models.Model):
    ...
    bisque_lot_id = fields.Many2one(
        "stock.lot",
        required=True,
        domain="[('product_id', '=', bisque_product_id)]",
        check_company=True,
    )
    ...
    def _check_selection(self):
        for line in self:
            ...
            if any(
                ...
                raise ValidationError(
                    _("The selected products, lot and glazing session must share a company.")
                )
            ...
            if line.bisque_product_id not in line.bom_id.bom_line_ids.product_id:
                raise ValidationError(
                    _("The glazing bill of materials does not consume the bisque ware.")
                )
    ...
    def _reserve_exact_move_lot(self, move, lot, quantity, location):
        ...
        if (
            ...
            raise UserError(
                _(
                    "The exact %(product)s lot could not be reserved; no substitute was used.",
                    product=move.product_id.display_name,
                )
            )
    ...
    def _reserve_tracked_materials(self, production, bisque_move):
        ...
        for move in tracked_moves:
            ...
            allocations = self.allocation_ids.filtered(
                lambda allocation, product=move.product_id: allocation.product_id == product
            )
            total = sum(
            ...
            if (
                ...
                raise UserError(
                    _(
                        "%(product)s allocations must total exactly %(quantity)s %(uom)s.",
                        product=move.product_id.display_name,
