...
class MrpProduction(models.Model):
    ...
    def action_confirm(self):
        for production in self:
            ...
            if production.mb_bom_revision_id and production.mb_bom_revision_id != bom:
                raise UserError(_("The recipe revision snapshot on %s cannot be replaced.", production.name))
        ...
        for production in self:
            ...
            if progressed or loaded:
                raise UserError(_(
                    ...
                    "a physical firing.", production.name,
                ))
    ...
    def _compute_state(self):
        ...
        for production in self:
            ...
            finished_draft = all(
                move.state == "draft" for move in production.move_finished_ids
            )
            if production.state == "cancel" and raw_draft and finished_draft:
                production.state = "draft"
            elif (production.state == "confirmed" and not production.move_raw_ids
                  and production.move_finished_ids and finished_draft):
                production.state = "draft"
        ...
        for production in self:
            ...
            if production.mb_bom_revision_id:
                ...
                conflicts = lots.filtered(
                    ...
                    lot.mb_bom_revision_id
                    and lot.mb_bom_revision_id != recipe_revision
                )
                if conflicts:
                    raise UserError(_(
    ...
    def _mb_check_food_contact(self):
        ...
        for production in self.filtered(lambda mo: mo.product_id.mb_food_contact):
            ...
            internally_produced = self.env["stock.move.line"].search([
                ("lot_id", "in", glaze_lots.ids),
                ("move_id.production_id", "!=", False),
                ("move_id.state", "=", "done"),
            ]).lot_id
            missing = internally_produced.filtered(
                lambda lot: not lot.mb_bom_revision_id
                or not lot.mb_bom_revision_id.mb_approved_at
            )
            if missing:
                raise UserError(_(
    ...
    def button_mark_done(self):
        for production in self.filtered(lambda mo: mo.bom_id.mb_is_glaze_recipe):
            if (not production.mb_bom_revision_id
