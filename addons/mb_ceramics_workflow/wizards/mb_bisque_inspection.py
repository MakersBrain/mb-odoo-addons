...
class MbBisqueInspection(models.TransientModel):
    ...
    selected_quantity = fields.Float(related="production_id.product_qty", readonly=True)
    ...
    def _complete_total_loss(self):
        ...
        if production.state != "done":
            raise UserError(
                _("The bisque order needs manual review before its loss can be scrapped.")
            )
    ...
    def action_confirm(self):
        ...
        if (
            ...
            raise UserError(
                _("Accepted and process-loss quantities must equal the selected green ware.")
            )
        firing_orders = production.workorder_ids.filtered(
            lambda workorder: workorder.operation_id.mb_kiln_program_id.kind == "bisque"
        )
        if not firing_orders or any(order.mb_firing_id.state != "done" for order in firing_orders):
        ...
        if lots:
            return (
                lots[:1]
                .with_context(mb_wip_quantity=self.accepted_quantity)
