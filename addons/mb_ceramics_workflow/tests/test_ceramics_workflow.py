...
class TestCeramicsWorkflow(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.costing_available = "property_cost_method" in cls.env["product.category"]._fields
        ...
        cls.article.product_tmpl_id.mb_second_product_tmpl_id = cls.second.product_tmpl_id
        ...
        if cls.ceramics_cost_category:
            (
                ...
                | cls.glaze_additive
                | cls.glaze_water
            ).categ_id = cls.ceramics_cost_category
        ...
        cls.env["stock.quant"]._update_available_quantity(cls.glaze_additive, cls.stock, 10)
        cls.env["stock.quant"]._update_available_quantity(cls.glaze_water, cls.stock, 10)
        ...
        cls.board_type = cls.env.ref("mb_ceramics_workflow.mb_package_type_ware_board")
    ...
    def _produce_bisque(self, quantity=4, accepted=None, loss=0):
        ...
        session, production = self._start_bisque(throwing.line_ids.blank_lot_id, quantity)
        self._finish_until(production, self.bisque_only_fire_op)
        firing = self._fire_board(production, self.bisque_only_fire_op, self.bisque_program)
    ...
    def _fire_board(self, production, operation, program):
        workorder = production.workorder_ids.filtered(lambda order: order.operation_id == operation)
    ...
    def test_bisque_board_split_retains_workflow_session(self):
        ...
        session, production = self._start_bisque(throwing.line_ids.blank_lot_id, 4)
    ...
    def test_firing_rejects_wrong_program(self):
        ...
        _session, production = self._start_bisque(throwing.line_ids.blank_lot_id, 2)
    ...
    def test_food_contact_release_requires_tested_glaze_lot(self):
        ...
        _bisque_session, _bisque_mo, bisque_lot, _firing, _action = self._produce_bisque(quantity=2)
    ...
    def test_bisque_stock_boundary_and_wip_label(self):
        ...
        self.assertEqual(
            action["context"]["default_manual_values_json"],
            {
                "stage": "BISQUE",
                "quantity": "3.0",
            },
    ...
    def test_shared_firing_charges_one_physical_duration(self):
        throwing = self._throw(4)
        first_session, first = self._start_bisque(throwing.line_ids.blank_lot_id, 1)
        second_session, second = self._start_bisque(throwing.line_ids.blank_lot_id, 1)
        self._finish_until(first, self.bisque_only_fire_op)
        self._finish_until(second, self.bisque_only_fire_op)
        workorders = (first.workorder_ids | second.workorder_ids).filtered(
            lambda order: order.operation_id == self.bisque_only_fire_op
        )
        ...
        self.assertEqual(len(workorders.filtered(lambda order: order.duration_expected)), 1)
    ...
    def test_glazing_reserves_exact_multiple_glaze_lots(self):
        _bisque_session, bisque_mo, bisque_lot, _firing, _action = self._produce_bisque(quantity=4)
        ...
        glaze_move = production.move_raw_ids.filtered(lambda move: move.product_id == self.glaze)
        self.assertEqual(
            set(glaze_move.move_line_ids.lot_id.ids),
            {
                self.glaze_lot_a.id,
                self.glaze_lot_b.id,
            },
        ...
        self._finish_until(production, self.glazing_fire_op)
        glaze_firing = self._fire_board(production, self.glazing_fire_op, self.glaze_program)
    ...
    def test_glazing_allocation_mismatch_rolls_back(self):
        _session, _production, bisque_lot, _firing, _action = self._produce_bisque(quantity=2)
        glazing = self.env["mb.glazing.session"].create(
            {
                "board_id": self.board.id,
                "source_location_id": self.bisque_stock.id,
    ...
    def test_recipe_documents_are_available_from_every_work_order(self):
        ...
        for workorder in production.workorder_ids:
            ...
            self.assertIn(attachment.id, self.env["ir.attachment"].search(action["domain"]).ids)
