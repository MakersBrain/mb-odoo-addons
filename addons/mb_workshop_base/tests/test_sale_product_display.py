...
class TestSaleProductDisplay(TransactionCase):
    ...
    def test_template_selector_uses_pricelist_price(self):
        expected = format_amount(self.env, 45.0, self.pricelist.currency_id, trailing_zeroes=False)

        display_name = self.product_template.with_context(**self._sale_context()).display_name
    ...
    def test_general_selector_uses_standard_sales_price(self):
        ...
        display_name = self.product.with_context(mb_show_product_selector_price=True).display_name
