...
class MbKilnProgramSegment(models.Model):
    ...
    target_temperature = fields.Float(
        string="Target (deg C)",
        help="Where this segment ends. The next segment ramps from here.")
    soak_time = fields.Float(
        string="Hold (minutes)",
        help="How long the target is held before the next segment starts.")

    full_power = fields.Boolean(
        compute="_compute_schedule", string="Full power",
    ...
    start_temperature = fields.Float(
        compute="_compute_schedule", string="From (deg C)",
    ...
    duration_minutes = fields.Float(
        compute="_compute_schedule", string="Segment (min)")
    elapsed_minutes = fields.Float(
        compute="_compute_schedule", string="Elapsed (min)",
    ...
    @api.depends(
        "number", "ramp_rate", "target_temperature", "soak_time",
        ...
        for segment in self:
            if segment.ramp_rate < 0.0 or segment.soak_time < 0.0:
                raise ValidationError(
                    _("A segment cannot have a negative rate or hold."))
    ...
    def _values(self):
        ...
        return [
            ...
            for segment in self.sorted(
                key=lambda record: (record.number, record.id))
        ]
