...
class L10nFrMicroUrssafRate(models.Model):
	...
	@api.constrains(
		"date_from", "date_to", "levy", "category", "taxpayer_kind",
		"chamber_kind", "chamber_zone", "rate",
	)
	def _check_validity(self):
		for rule in self:
			...
			domain = [
				...
				"|", ("date_to", "=", False), ("date_to", ">=", rule.date_from),
			]
			if self.search_count(domain):
				raise ValidationError(_("Rate validity periods overlap for the same applicability key."))
	...
	def _check_validity(self):
		for rule in self:
			...
			if not 0 < rule.payable_coefficient <= 1:
				raise ValidationError(_("An ACRE payable coefficient must be above 0 and at most 1."))
			if self.search_count([
				("id", "!=", rule.id),
				("creation_date_from", "<=", rule.creation_date_to or fields.Date.to_date("9999-12-31")),
...
class L10nFrMicroUrssafAnnual(models.Model):
	...
	def create(self, values_list):
		for values in values_list:
			evidence = sum(abs(values.get(field, 0.0)) for field in (
				"urssaf_goods", "urssaf_services", "urssaf_total", "vat_global", "vat_services",
			))
			if evidence and (values.get("source", "computed") != "manual" or not values.get("manual_reason")):
				raise ValidationError(_("Opening annual evidence must be marked manual and include a reason."))
	...
	def write(self, values):
		...
		if values.get("source") == "computed" and not is_internal(self.env):
			raise ValidationError(_("Use Compute from filed evidence to create a computed annual snapshot."))
		evidence_fields = {
			"urssaf_goods", "urssaf_services", "urssaf_total",
		...
		if evidence_fields.intersection(values) and not is_internal(self.env):
			for record in self:
				...
				if source != "manual" or not reason:
					raise ValidationError(_("Editable annual evidence must be marked manual and include a reason."))
	...
	def action_compute(self):
		for record in self:
			...
			goods = sum(lines.filtered(lambda line: line.category == "bic_goods").mapped("declared_turnover"))
			services = sum(lines.filtered(lambda line: line.category != "bic_goods").mapped("declared_turnover"))
			...
			if not latest_filed:
				raise ValidationError(_("File the declaration covering 31 December before computing annual evidence."))
