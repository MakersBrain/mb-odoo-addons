...
class ResCompany(models.Model):
    ...
    l10n_fr_micro_acre_coefficient = fields.Float(
        string="ACRE payable coefficient",
        digits=(5, 4),
        default=1.0,
    ...
    l10n_fr_micro_accounting_responsible_id = fields.Many2one(
        "res.users",
        string="URSSAF accounting responsible",
        domain=[("share", "=", False)],
    )
    ...
    _urssaf_configuration_fields = {
        ...
        "l10n_fr_micro_chamber_kind",
        "l10n_fr_micro_chamber_zone",
    ...
    def write(self, values):
        if (
            "l10n_fr_micro_urssaf_tracking_start_date" in values
        ...
        if "l10n_fr_micro_depot_sale_closed_through" in values:
            ...
            new_horizon = (
                fields.Date.to_date(values["l10n_fr_micro_depot_sale_closed_through"])
                if values["l10n_fr_micro_depot_sale_closed_through"]
                else False
            )
            for company in self:
                if company.l10n_fr_micro_depot_sale_closed_through and (
                    not new_horizon or new_horizon < company.l10n_fr_micro_depot_sale_closed_through
                ):
                    raise ValidationError(
                        _("The permanent URSSAF depot-sale horizon cannot be reduced.")
                    )
        if (
            self._urssaf_configuration_fields.intersection(values)
            and not self.env.is_superuser()
            and not is_internal(self.env)
            and not self.env.user.has_group("account.group_account_manager")
        ):
            raise AccessError(
                _("Only an Accounting Administrator can change URSSAF configuration.")
            )
    ...
    def action_l10n_fr_micro_confirm_depot_sale_horizon(self):
        if not self.env.user.has_group("account.group_account_manager"):
            raise AccessError(
                _("Only an Accounting Administrator can confirm the depot-sale horizon.")
            )
    ...
    def _check_urssaf_configuration(self):
        for company in self:
            if (
                ...
                and company.l10n_fr_micro_urssaf_tracking_start_date
                and company.l10n_fr_micro_urssaf_tracking_start_date
                < company.l10n_fr_micro_activity_start_date
            ):
            ...
            for date_from, date_to, label in (
                ...
                if date_from and date_to and date_to < date_from:
                    raise ValidationError(
                        _("%(label)s end date precedes its start date.", label=label)
                    )
            if company.l10n_fr_micro_acre_granted:
                ...
                if not 0 < company.l10n_fr_micro_acre_coefficient <= 1:
                    raise ValidationError(
                        _("The ACRE payable coefficient must be above 0 and at most 1.")
                    )

    def _l10n_fr_micro_acre_coefficient_on(self, date):
        self.ensure_one()
        if (
            self.l10n_fr_micro_acre_granted
    ...
    def action_l10n_fr_micro_apply_acre_rule(self):
        for company in self:
            ...
            rule = self.env["l10n.fr.micro.urssaf.acre.rule"].search(
                ...
                order="creation_date_from desc",
                limit=1,
            )
            if not rule:
                raise ValidationError(
                    _("No ACRE creation-date rule applies to this activity start date.")
                )
