...
class L10nFrMicroUrssafDeclaration(models.Model):
    ...
    line_ids = fields.One2many(
        "l10n.fr.micro.urssaf.declaration.line",
        "declaration_id",
        copy=False,
    )
    ...
    def default_get(self, field_names):
        ...
        if previous:
            ...
            date_from = (
                company.l10n_fr_micro_urssaf_tracking_start_date
                or company.l10n_fr_micro_activity_start_date
    ...
    def _compute_due_date(self):
        for declaration in self:
            declaration.due_date = (
                declaration.date_to
                and (declaration.date_to + relativedelta(months=1, day=31))
                or False
            )
    ...
    def _check_period(self):
        for declaration in self:
            if (
                ...
                and declaration.date_to
                and declaration.date_to < declaration.date_from
            ):
            ...
            if previous and declaration.date_from != previous.date_to + relativedelta(days=1):
                raise ValidationError(
                    _(
                        ...
                        expected=previous.date_to + relativedelta(days=1),
    ...
    def _check_manager(self):
        if not self.env.is_superuser() and not self.env.user.has_group(
            ...
            raise AccessError(
                _("Only an Accounting Administrator can perform this declaration action.")
            )
    ...
    def write(self, values):
        ...
        if "state" in values and not is_internal(self.env):
            raise AccessError(
                _("Use the manager-only file or reset action to change declaration state.")
            )
        if filed and not reset_reason_only and not is_internal(self.env):
            raise UserError(
                _("A filed URSSAF declaration is immutable. Reset it to draft with a reason first.")
            )
    ...
    def _foreign_details(self, company_amount, move_line=None, partial=None, origin=None):
        ...
        company_currency = (
            origin.company_id.currency_id
            if origin
            else (move_line.company_id.currency_id if move_line else self.env.company.currency_id)
        )
    ...
    def _pos_recognition_date(self, order):
        ...
        return self._company_local_date(
            order.company_id,
            max(payment_dates) if payment_dates else order.date_order,
        )
        ...
        for order in orders:
            ...
            for line in order.lines:
                categories = set(
                    filter(
                        None,
                        line.tax_ids_after_fiscal_position.mapped("l10n_fr_micro_urssaf_category"),
                    )
                )
    ...
    def _reconciliation_events(self, company, date_from, date_to):
        ...
        for invoice in invoices:
            ...
            terms = invoice.line_ids.filtered(lambda line: line.account_type == "asset_receivable")
            total = abs(sum(terms.mapped("balance")))
            if company.currency_id.is_zero(total):
                continue
            partials = (
                (terms.matched_debit_ids | terms.matched_credit_ids)
                .filtered(lambda partial: date_from <= partial.max_date <= date_to)
                .sorted(lambda partial: (partial.max_date, partial.id))
            )
            all_partials = (
                (terms.matched_debit_ids | terms.matched_credit_ids)
                .filtered(lambda partial: partial.max_date <= date_to)
                .sorted(lambda partial: (partial.max_date, partial.id))
            )
    ...
    def _pos_daily_summaries(self):
        ...
        for source in self.source_ids.filtered(lambda item: item.engine == "pos"):
            ...
            group = groups.setdefault(
                key,
                {
                    ...
                    "method": dict(source._fields["receipt_method"].selection).get(
                        source.receipt_method
                    ),
        ...
        return sorted(
            result,
            key=lambda group: (group["date"], group["method"] or "", group["category"] or ""),
        )
    ...
    def _vat_goods_events_for_line(self, line):
        ...
        for prior_line in invoice_lines:
            ...
            if prior_key < line_key:
                prior_quantity += abs(
                    prior_line.product_uom_id._compute_quantity(
                        prior_line.quantity,
                        line.product_uom_id,
                    )
                )
    ...
    def _vat_threshold_events(self, company, date_from, date_to):
        events = [
            event
            for event in self._recognition_events(company, date_from, date_to)
        ...
        for order in orders:
            ...
            for line in order.lines:
                if "bic_goods" in line.tax_ids_after_fiscal_position.mapped(
    ...
    def _unclassified_anomalies(self):
        ...
        lines = self.env["account.move.line"].search(
            [
                ...
                (
                    "date",
                    ">=",
                    self.company_id.l10n_fr_micro_urssaf_tracking_start_date or self.date_from,
                ),
                ("date", "<=", self.date_to),
            ]
        )
        return [
            _(
                "Posted sale line %(move)s / %(line)s has no URSSAF tax category.",
                move=line.move_id.name,
                line=line.name,
            )
            for line in lines
            if not self._categories_on_line(line)
        ]
    ...
    def _missing_vat_dates(self):
        ...
        return [
            _(
                "Goods line %(move)s / %(line)s has no provable VAT-threshold delivery date.",
                move=line.move_id.name,
                line=line.name,
            )
            for line in lines
            if not self._vat_operation_date(line)
        ]
    ...
    def _chamber_applies(self, date, category=None):
        ...
        if (
            category == "bnc"
            or company.l10n_fr_micro_chamber_kind == "none"
    ...
    def _source_rate_values(self, event):
        ...
        chamber = (
            Rate.rate_for("chamber", category, date, self.company_id)
            if self._chamber_applies(date, category)
            else Rate
        )
        liberatoire = (
            Rate.rate_for("liberatoire", category, date, self.company_id)
            if self.company_id._l10n_fr_micro_has_versement_on(date)
            else Rate
        )
        ...
        return {
            ...
            "cotisation_amount": self.currency_id.round(
                amount * (cotisation.rate if cotisation else 0.0) / 100 * acre
            ),
            ...
            "chamber_amount": self.currency_id.round(
                amount * (chamber.rate if chamber else 0.0) / 100
            ),
            "liberatoire_rate_id": liberatoire.id or False,
            "liberatoire_rate": liberatoire.rate if liberatoire else 0.0,
            "liberatoire_amount": self.currency_id.round(
                amount * (liberatoire.rate if liberatoire else 0.0) / 100
            ),
        }
    ...
    def _missing_rate_anomalies(self, event, rate_values):
        ...
        if (
            self._chamber_applies(event["date"], event["category"])
            and not rate_values["chamber_rate_id"]
        ):
            missing.append(_("consular-chamber"))
        if (
            self.company_id._l10n_fr_micro_has_versement_on(event["date"])
    ...
    def _threshold_values(self, all_events):
        ...
        micro_events = [
            event for event in all_events if year_start <= event["date"] <= self.date_to
        ]
        ...
        micro_services = sum(
            event["amount"] for event in micro_events if event["category"] != "bic_goods"
        )
        vat_global = sum(event["amount"] for event in vat_events)
        vat_services = sum(
            event["amount"] for event in vat_events if event["category"] != "bic_goods"
        )
        ...
        if threshold:
            ...
            status = [
                _(
                    "Micro: %(global).2f / %(global_limit).2f globally; %(services).2f / %(service_limit).2f services.",
                    **{
                        "global": micro_global,
                        "global_limit": micro_global_limit,
                        "services": micro_services,
                        "service_limit": micro_service_limit,
                    },
                ),
                _(
                    "VAT franchise: %(global).2f (base %(global_base).2f; major %(global_major).2f); services %(services).2f (base %(service_base).2f; major %(service_major).2f).",
                    **{
                        "global": vat_global,
                        "global_base": vat_global_base,
                        "global_major": vat_global_major,
                        "services": vat_services,
                        "service_base": vat_service_base,
                        "service_major": vat_service_major,
                    },
                ),
            ]
            previous = self.env["l10n.fr.micro.urssaf.annual"].search(
            ...
            if previous:
                status.append(
                    _(
                        ...
                        micro=previous.urssaf_total,
                        services=previous.urssaf_services,
                        vat=previous.vat_global,
                        vat_services=previous.vat_services,
                    )
                )
                previous_threshold = (
                    self.env["l10n.fr.micro.urssaf.threshold"].threshold_for(
                        year_start - relativedelta(days=1)
                    )
                    or threshold
                )
                if (
                    previous.vat_global > previous_threshold.vat_global_base
                    or previous.vat_services > previous_threshold.vat_service_base
                ):
                    status.append(
                        _(
                            "The prior year exceeded a VAT-franchise base threshold; verify continued eligibility."
                        )
                    )
                if (
                    ...
                    status.append(
                        _(
                            "The micro-regime ceiling has been exceeded in two consecutive years; review the mandatory regime exit."
                        )
                    )
        self.write(
            {
        ...
        if threshold and (vat_global > vat_global_major or vat_services > vat_service_major):
    ...
    def _ensure_vat_activity(self, crossing_date):
        ...
        values = {
            ...
            "summary": summary,
            "note": _(
                "Verify the source events and switch VAT from the actual crossing date: %s",
                crossing_date,
            ),
    ...
    def action_compute(self):
        for declaration in self:
            ...
            if (
                not company.l10n_fr_micro_urssaf_tracking_start_date
            ...
            for category, _label in URSSAF_CATEGORIES:
                ...
                lines[category] = (
                    self.env["l10n.fr.micro.urssaf.declaration.line"]
            ...
            for event in all_events:
                    ...
                    if claim.declaration_state != "filed":
                        anomalies.append(
                            _(
                                ...
                                declaration=claim.declaration_id.display_name,
                            )
                        )
                    continue
                values = {
                ...
                if event["receipt_method"] == "unknown":
                    anomalies.append(
                        _(
                            "Receipt method is unproven for %(description)s.",
                            description=event["description"],
                        )
                    )
            for line in declaration.line_ids:
                ...
                if line.computed_turnover < 0:
                    anomalies.append(
                        _(
                            ...
                            category=dict(URSSAF_CATEGORIES)[line.category],
                            amount=line.computed_turnover,
                        )
                    )
            declaration.write(
                {
    ...
    def action_file(self):
        ...
        for declaration in self:
            ...
            declaration.company_id._l10n_fr_micro_advance_depot_sale_horizon(declaration.date_to)
    ...
    def action_print(self):
        ...
        if not self.line_ids:
            self.action_compute()
        return self.env.ref("l10n_fr_micro_urssaf.action_report_urssaf_declaration").report_action(
            self
        )
...
class L10nFrMicroUrssafDeclarationLine(models.Model):
    ...
    def create(self, values_list):
        ...
        if declarations.filtered(
            lambda declaration: declaration.state == "filed"
    ...
    def write(self, values):
        if self.filtered(lambda line: line.declaration_state == "filed") and not is_internal(
            self.env
        ):
            raise UserError(_("Filed declaration lines are immutable."))
    ...
    def _refresh_amounts(self):
        for line in self:
            current = sum(
                line.source_ids.filtered(lambda source: not source.is_prior_period).mapped("amount")
            )
            ...
            for source in line.source_ids:
                bases["cotisation"][(source.cotisation_rate, source.acre_coefficient)] += (
                    source.amount
                )
            ...
            if line.manual_adjustment:
                event = {
                    "date": line.declaration_id.date_to,
                    "category": line.category,
                    "amount": line.manual_adjustment,
                }
                rates = line.declaration_id._source_rate_values(event)
                bases["cotisation"][(rates["cotisation_rate"], rates["acre_coefficient"])] += (
                    line.manual_adjustment
                )
            ...
            summaries = sorted(
                {
                    _(
                        "%(date)s: cot. %(cot)s%% × ACRE %(acre)s; CFP %(cfp)s%%; chamber %(chamber)s%%; VFL %(vfl)s%%",
                        date=source.recognition_date,
                        cot=source.cotisation_rate,
                        acre=source.acre_coefficient,
                        cfp=source.cfp_rate,
                        chamber=source.chamber_rate,
                        vfl=source.liberatoire_rate,
                    )
                    for source in line.source_ids
                }
            )
...
class L10nFrMicroUrssafDeclarationSource(models.Model):
    ...
    source_currency_id = fields.Many2one(
        "res.currency", string="Receipt currency", ondelete="restrict"
    )
    source_amount_currency = fields.Monetary(
        string="Receipt foreign amount", currency_field="source_currency_id"
    )
    ...
    cotisation_rate_id = fields.Many2one(
        "l10n.fr.micro.urssaf.rate", string="Contribution rule", ondelete="restrict"
    )
    ...
    cfp_rate_id = fields.Many2one(
        "l10n.fr.micro.urssaf.rate", string="CFP rule", ondelete="restrict"
    )
    cfp_rate = fields.Float(string="CFP rate (%)", digits=(8, 4))
    cfp_amount = fields.Monetary()
    chamber_rate_id = fields.Many2one(
        "l10n.fr.micro.urssaf.rate", string="Chamber-tax rule", ondelete="restrict"
    )
    ...
    liberatoire_rate_id = fields.Many2one(
        ...
        string="Flat-rate income tax payment rule",
        ondelete="restrict",
    )
    liberatoire_rate = fields.Float(
        string="Flat-rate income tax payment rate (%)",
        digits=(8, 4),
    )
    ...
    def _check_source_integrity(self):
        for source in self:
            if source.declaration_line_id.declaration_id != source.declaration_id:
                raise ValidationError(
                    _("Receipt evidence and its turnover box must belong to the same declaration.")
                )
    ...
    def write(self, values):
        if self.filtered(lambda source: source.declaration_state == "filed") and not is_internal(
            self.env
        ):
            raise UserError(_("Filed declaration sources are immutable."))
        if (
