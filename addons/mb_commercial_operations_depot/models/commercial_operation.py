...
class MbCommercialOperation(models.Model):
    ...
    ], required=True, default="operation_only", tracking=True)
    ...
    def _recovery_bounds(self):
        ...
        if self.recovery_scope == "until_next_refill":
            next_refill = self.search([
                ("id", "!=", self.id), ("contract_id", "=", self.contract_id.id),
                ("operation_type", "=", "depot_refill"),
                ("state", "in", ("approved", "scheduled", "in_progress", "done", "financially_closed")),
                ("planned_start", ">", start),
            ], order="planned_start, id", limit=1)
    ...
    def _check_recovery_window(self):
        for operation in self.filtered(
            ...
            candidates = self.search([
                ("id", "!=", operation.id), ("contract_id", "=", operation.contract_id.id),
            ...
            if any(
                ...
                raise ValidationError(_("Depot recovery windows for the same contract cannot overlap."))

    def _get_planning_warnings(self, scenario=None):
        warnings = super()._get_planning_warnings(scenario)
        if self.recovery_scope == "contract_period" and not (self.recovery_date_from and self.recovery_date_to):
            warnings.append(("missing_recovery_period", "blocking", _("Choose the depot recovery-period boundaries.")))
        return warnings

    def _get_depot_comparison_items(self):
        self.ensure_one()
        items = []
        if self.recovery_scope not in ("until_next_refill", "contract_period") or not self.contract_id:
            return items
        ...
        for line in self.env["mb.depot.sale.report.line"].search(domain):
            items.append({
                "model": line._name, "res_id": line.id, "component": "revenue",
                "date": fields.Date.to_date(line.sold_at), "amount": line.net_line_amount,
