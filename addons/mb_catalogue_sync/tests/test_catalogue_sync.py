...
PAYLOAD = {
    ...
    "offers": [
        {"source_id": "ceradel", "supplier_name": "Hot Tamale",
         "supplier_reference": "1425102", "price": 26.88, "currency": "EUR",
         "vat_status": "inclusive", "package_quantity": 473.0, "package_unit": "ml"},
        {"source_id": "ceradel", "supplier_name": "Hot Tamale",
         "supplier_reference": "1425101", "price": 14.99, "currency": "EUR",
         "vat_status": "inclusive", "package_quantity": 236.0, "package_unit": "ml"},
        # The same 16 oz jar, published by another shop as 472 ml.
        {"source_id": "1240-design", "supplier_name": "Hot Tamale Ceramic Glaze",
         "supplier_reference": "HT472", "price": 23.77, "currency": "EUR",
         "vat_status": "inclusive", "package_quantity": 472.0, "package_unit": "ml"},
        # A source with no VAT basis published, and none configured on the vendor.
        {"source_id": "cerama-shop", "supplier_name": "Hot Tamale Formato: 236 ml",
         "supplier_reference": "CS236", "price": 14.50, "currency": "EUR",
         "vat_status": None, "package_quantity": 236.0, "package_unit": "ml"},
        # A source this workshop does not buy from.
        {"source_id": "hiclay", "supplier_name": "STROKE & COAT SC-74 HOT TAMALE",
         "supplier_reference": "SC74", "price": 22.0, "currency": "USD",
         "vat_status": "exclusive", "package_quantity": 1.0, "package_unit": "pint"},
    ],
}
...
class TestCatalogueSync(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        supplier.create({"source_id": "ceradel", "partner_id": cls.ceradel.id,
                         "vat_status": "inclusive", "vat_rate": 20.0, "delay": 5})
        supplier.create({"source_id": "1240-design", "partner_id": cls.design.id,
    ...
    def test_manufacturer_code_is_the_internal_reference_on_every_pack(self):
        ...
        self.assertEqual(template.product_variant_ids.mapped("default_code"),
                         ["SC74", "SC74"])
        for variant in template.product_variant_ids:
            self.assertIn("SC74", variant.display_name)

    def test_a_single_pack_product_shows_the_code_on_the_template(self):
        payload = dict(PAYLOAD, canonical_product_id="single-pack-0001",
    ...
    def test_category_and_code_are_filled_but_never_overwritten(self):
        ...
        self.assertEqual(set(template.product_variant_ids.mapped("default_code")),
                         {"SHELF-1"})
    ...
    def test_reimport_is_idempotent(self):
        ...
        self.assertEqual(
            self.template.search_count([("mb_manufacturer_sku", "=", "SC74")]), 1)
        self.assertEqual(
    ...
    def test_an_unmapped_source_produces_no_vendor(self):
        ...
        vendors = self.env["product.supplierinfo"].search(
            [("product_tmpl_id", "=", template.id)]).mapped("partner_id")
