...
class TestShopImport(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.source = cls.env["mb.shop.source"].search(
            ...
            limit=1,
        ) or cls.env["mb.shop.source"].create(
            {
    ...
    def test_parse_is_review_only_and_classifies_services_and_stock(self):
        ...
        self.assertEqual(
            batch.file_sha256, __import__("hashlib").sha256(FIXTURE.read_bytes()).hexdigest()
        )
    ...
    def test_import_sets_product_policy_bindings_and_exact_stock(self):
        ...
        self.assertEqual(
            self.env["mb.shop.product.binding"].search_count(
                [
                    ("source_id", "=", self.source.id),
                ]
            ),
        ...
        physical = batch.line_ids.filtered(lambda line: not line.is_service).mapped(
            "ingested_product_id"
        )
        ...
        blue = batch.line_ids.filtered(
            lambda line: line.variant_title == "Bleu"
        ).ingested_product_id
        self.assertEqual(self.env["stock.quant"]._get_available_quantity(blue, self.location), 2)
        untracked = batch.line_ids.filtered(
            lambda line: line.name == "Fouet matcha"
        ).ingested_product_id
        self.assertEqual(
            self.env["stock.quant"]._get_available_quantity(untracked, self.location), 0
        )
    ...
    def test_unselected_errors_do_not_block_valid_selected_lines(self):
        ...
        batch.action_validate()
        self.assertEqual(
            len(batch.line_ids.filtered(lambda line: line.validation_status == "error")), 2
        )
    ...
    def test_ingestion_failure_rolls_back_products_but_persists_failure(self):
        ...
        before = self.env["product.template"].search_count([])
        with patch.object(
            type(batch), "_set_stock", side_effect=ValidationError("synthetic failure")
        ):
            batch.action_import_selected()
        ...
        self.assertEqual(self.env["product.template"].search_count([]), before)
        self.assertFalse(
            self.env["mb.shop.product.binding"].search(
                [
                    ("source_id", "=", self.source.id),
                ]
            )
    ...
    def test_unexpected_image_failure_does_not_undo_product_ingestion(self):
        ...
        self.assertTrue(
            all("secret internal endpoint" not in line.image_failure for line in failed)
        )
    ...
    def test_company_rules_hide_sources_batches_lines_and_bindings(self):
        ...
        for record in (source, batch, line, binding):
            restricted = record.with_user(user).with_context(allowed_company_ids=[self.company.id])
