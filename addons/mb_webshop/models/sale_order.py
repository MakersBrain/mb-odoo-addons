...
class SaleOrder(models.Model):
    ...
    def _mb_original_delivered_quantity(self, line):
        ...
        return sum(
            move.product_uom._compute_quantity(move.quantity, line.product_uom_id, round=False)
            for move in moves
        )
    ...
    def _mb_returnable_quantity(self, line):
        ...
        line.ensure_one()
        requested = sum(
            self.env["mb.webshop.return.line"]
            .sudo()
            .search(
                [
    ...
    def _mb_returnable_lines(self):
        ...
        return self.order_line.filtered(
            lambda line: (
                ...
                and line.product_uom_id.compare(self._mb_returnable_quantity(line), 0) > 0
            )
        )
    ...
    def _mb_hold_eligible(self, product):
        self.ensure_one()
        return bool(
            self.website_id and product.is_storable and not product.allow_out_of_stock_order
        )
    ...
    def _get_free_qty(self, product):
        ...
        if self._mb_hold_eligible(product):
            ...
            if hold:
                ...
                available += product.uom_id._compute_quantity(hold.move_id.quantity, product.uom_id)
        return available
    ...
    def _mb_cart_quantity(self, product):
        self.ensure_one()
        return sum(
            line.product_uom_id._compute_quantity(line.product_uom_qty, product.uom_id)
    ...
    def action_confirm(self):
        for order in self.filtered("website_id"):
            ...
            holds = (
                self.env["mb.webshop.stock.hold"]
                .sudo()
                .search([("order_id", "=", order.id), ("state", "=", "active")])
            )
