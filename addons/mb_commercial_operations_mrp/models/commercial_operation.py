...
class MbCommercialOperation(models.Model):
    ...
    manufacturing_order_ids = fields.One2many(
        "mrp.production", "mb_commercial_operation_id", string="Manufacturing Orders",
    )
...
class MbMarketStockPlanLine(models.Model):
    ...
    production_ids = fields.One2many(
        "mrp.production", "mb_market_stock_plan_line_id", string="Manufacturing Orders",
    )

    def _prepare_supply(self):
        super()._prepare_supply()
        for line in self.filtered(lambda item: item.supply_method == "manufacture"):
            product = line.product_id if line.target_type == "product" else line.manufacturing_product_id
            if not product:
            ...
            if not bom:
                raise ValidationError(_("No bill of materials is available for %(product)s.", product=product.display_name))
            ...
            production = self.env["mrp.production"].with_context(
                default_date_deadline=operation.stock_preparation_deadline or operation.planned_start
            ).create({
                "product_id": product.id,
        ...
        for line in self.filtered(lambda item: item.supply_method == "manufacture"):
            ...
            if orders:
                if any(order.state == "draft" for order in orders):
                    state, note = "supply_proposed", _("Draft manufacturing supply awaits review and confirmation.")
                    ...
                    state, note = "ready_to_pick", _("Manufacturing is complete; refresh stock availability before preparation.")
                else:
                    cutoff = line.operation_id.stock_preparation_deadline or line.operation_id.planned_start
                    at_risk = any(
                        order.date_finished and order.date_finished > cutoff
                        or order.components_availability_state == "unavailable"
                        for order in orders
                    )
                    state = "at_risk" if at_risk else "supply_confirmed"
                    note = _("Confirmed supply is late or lacks components.") if at_risk else _("Manufacturing supply is confirmed.")
