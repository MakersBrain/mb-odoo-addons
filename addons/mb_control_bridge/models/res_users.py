...
class ResUsers(models.Model):
    ...
    def mb_replay_erasure(self, payload):
        ...
        user.partner_id.write({
            key: value for key, value in partner_values.items()
            if key in user.partner_id._fields
        })
    ...
    def mb_export_personal_data(self, payload):
        ...
        add_dataset(
            "users", "res.users", [("id", "=", user.id)],
            ["login", "name", "email", "active", "lang", "tz",
             "mb_control_role", "mb_control_membership_epoch",
             "mb_rauthy_subject", "create_date", "write_date"]
        )
        add_dataset(
            "partners", "res.partner", [("id", "child_of", partner.id)],
            ["name", "email", "phone", "mobile", "street", "street2",
             "zip", "city", "state_id", "country_id", "vat", "website",
             "lang", "tz", "comment", "create_date", "write_date"]
        )
        business = {
            "account_moves": (
                "account.move", [("commercial_partner_id", "=", partner.id)],
                ["name", "move_type", "date", "invoice_date", "invoice_date_due",
                 "ref", "payment_reference", "amount_untaxed", "amount_tax",
                 "amount_total", "state", "payment_state", "create_date", "write_date"]
            ),
            "sale_orders": (
                "sale.order", [("partner_id", "child_of", partner.id)],
                ["name", "client_order_ref", "date_order", "state", "amount_untaxed",
                 "amount_tax", "amount_total", "note", "create_date", "write_date"]
            ),
            "purchase_orders": (
                "purchase.order", [("partner_id", "child_of", partner.id)],
                ["name", "partner_ref", "date_order", "date_approve", "state",
                 "amount_untaxed", "amount_tax", "amount_total", "notes",
                 "create_date", "write_date"]
            ),
            "pos_orders": (
                "pos.order", [("partner_id", "child_of", partner.id)],
                ["name", "pos_reference", "date_order", "state", "amount_paid",
                 "amount_total", "amount_tax", "create_date", "write_date"]
            ),
            "stock_pickings": (
                "stock.picking", [("partner_id", "child_of", partner.id)],
                ["name", "origin", "state", "scheduled_date", "date_done",
            ...
            "messages": (
                ...
                ["model", "res_id", "message_type", "subject", "body",
                 "email_from", "author_id", "partner_ids", "date",
                 "create_date", "write_date"]
            ),
        }
        ...
        lines = {
            "account_move_lines": (
                ...
                ["name", "display_type", "quantity", "price_unit", "discount",
                 "price_subtotal", "price_total", "debit", "credit", "balance",
                 "date", "date_maturity", "product_id", "create_date", "write_date"]
            ),
            "sale_order_lines": (
                ...
                ["name", "product_id", "product_uom_qty", "qty_delivered",
                 ...
                 "price_tax", "price_total", "create_date", "write_date"]
            ),
            "purchase_order_lines": (
                "purchase.order.line", "order_id", "purchase.order",
                ["name", "product_id", "product_qty", "qty_received", "qty_invoiced",
                 "price_unit", "price_subtotal", "price_tax", "price_total",
                 "date_planned", "create_date", "write_date"]
            ),
            "pos_order_lines": (
                "pos.order.line", "order_id", "pos.order",
                ["full_product_name", "product_id", "qty", "price_unit", "discount",
                 "price_subtotal", "price_subtotal_incl", "create_date", "write_date"]
            ),
            "stock_moves": (
                "stock.move", "picking_id", "stock.picking",
                ["name", "product_id", "product_uom_qty", "quantity", "state",
                 "origin", "date", "deadline", "create_date", "write_date"]
            ),
        }
        ...
        for model_name, record_ids in sorted(related.items()):
            ...
            attachment_domain = clause if not attachment_domain else ["|"] + attachment_domain + clause
        remaining = EXPORT_BYTES_LIMIT - len(json.dumps(
            datasets, ensure_ascii=False, separators=(",", ":")
        ).encode()) - 4096
        attachments = self._mb_export_model(
            "ir.attachment", attachment_domain or [("id", "=", 0)],
            ["name", "res_model", "res_id", "mimetype", "file_size", "checksum",
             "description", "datas", "create_date", "write_date"], None, remaining
        )
        result = {
            "format": "makersbrain-odoo-subject-export-v1",
            "workshop_id": workshop_id,
        ...
        if len(json.dumps(result, ensure_ascii=False, separators=(",", ":")).encode()) > EXPORT_BYTES_LIMIT:
            raise ValidationError(_("the Odoo subject export exceeds the secure export limit"))
        return result

    @api.model
    def _mb_export_model(
        self, model_name, domain, field_names, related, maximum_bytes
    ):
        ...
        if count > EXPORT_RECORD_LIMIT:
            raise ValidationError(_("the %(model)s subject export exceeds %(limit)s records",
                                    model=model_name, limit=EXPORT_RECORD_LIMIT))
