...
export async function loadPrinterDefinitions() {
  ...
  if (!response.ok) throw new Error(`Unable to load Phomemo definitions (${response.status})`);
  const builtinPrinters = await response.json();
  _builtinDefinitions = builtinPrinters.printers || [];
  _loaded = true;
  _rebuildDefinitions();
}
