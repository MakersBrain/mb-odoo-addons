...
class MbLabelPrintWizard(models.TransientModel):
    ...
    product_id = fields.Many2one(
        "product.product", string="Variant", required=True,
        domain="[('product_tmpl_id', '=', product_tmpl_id)]")
    lot_id = fields.Many2one(
        "stock.lot", string="Lot or Serial", domain="[('product_id', '=', product_id)]")
    template_id = fields.Many2one(
        "mb.label.template", required=True,
    ...
    def default_get(self, names):
        ...
        template = self.env["mb.label.template"].search([
            ("company_id", "=", self.env.company.id), ("is_default", "=", True),
            ("active", "=", True)], limit=1)
        if not template:
            template = self.env["mb.label.template"].search([
        ...
        if product_id:
            values.setdefault("product_tmpl_id", self.env["product.product"].browse(product_id).product_tmpl_id.id)
        return values
    ...
    def _create_job(self):
        ...
        payload = self.env["mb.label.print.job"].create_rendered(
            self.product_id.id, self.lot_id.id, self.template_id.id,
            self.copies, self.manual_values_json or {})
