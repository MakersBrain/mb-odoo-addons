...
class MbLabelQrAlias(models.Model):
        ...
        for record in self.filtered("lot_id"):
            if record.lot_id.product_id != record.product_id:
                raise ValidationError(
                    _("The lot or serial does not belong to the selected product.")
                )
    ...
    def mint(self, value, product_id, lot_id, template_version_id):
        ...
        if existing:
            if existing.product_id.id != product_id or (existing.lot_id.id or False) != (
                lot_id or False
            ):
                raise ValidationError(
                    _("QR collision: '%s' already identifies another product or piece.", value)
                )
