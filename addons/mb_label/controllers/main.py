...
class MbLabelController(http.Controller):
    ...
    def browser_print(self, job_id, autoprint=0):
        ...
        copies = "".join(
            ...
            % (job.id, escape(job.name)) for _copy in range(job.copies))
        script = "<script>addEventListener('load',()=>window.print())</script>" if str(autoprint) == "1" else ""
...
@media screen { body { background: #ddd; } .label { margin: 10px auto; box-shadow: 0 2px 10px #777; } }
            ...
            escape(job.name), job.width_mm, job.height_mm, job.width_mm,
            job.height_mm, copies, script)
