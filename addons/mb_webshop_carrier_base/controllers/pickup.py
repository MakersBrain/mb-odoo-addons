...
class MBPickupDelivery(Delivery):
    ...
    def website_sale_get_pickup_locations(self, zip_code=None, **kwargs):
        ...
        return order.with_context(mb_pickup_rate_identity=_rate_identity())._get_pickup_locations(
            zip_code, country, **kwargs
        )
    ...
    def website_sale_set_pickup_location(self, pickup_location_data):
        ...
        if not order:
            raise AccessDenied()
        order.with_context(mb_pickup_rate_identity=_rate_identity())._set_pickup_location(
