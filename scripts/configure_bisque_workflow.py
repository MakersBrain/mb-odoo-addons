...
products = {
    code: one_product(code)
    for code in STAGE_BY_DEFAULT_CODE
}
invalid_products = [
    code
...
validated_boms = {
    bisque_code: validate_bisque_bom(
        products[bisque_code], products[green_code]
    )
...
print("CLASSIFIED", {
    code: product.product_tmpl_id.mb_ceramics_stage
    for code, product in products.items()
})
print("VALIDATED_BOMS", {
    code: bom.display_name
    for code, bom in validated_boms.items()
})
