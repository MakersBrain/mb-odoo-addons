...
depot = env["stock.location"].search([("complete_name", "=", f"Dépôts/{GALLERY}")], limit=1)
...
if not prod:
    prod = env["product.product"].create(
        {
            "name": "Bol grès émaillé Ø14",
            "default_code": "BOL-G14",
            "type": "consu",
            "is_storable": True,
            "tracking": "serial",
            "list_price": 45.0,
...
print(f"product: {prod.display_name}  list {prod.list_price}  invoice_policy={prod.invoice_policy}")

# --- keep the depot stocked -------------------------------------------------
fresh = []
for i in range(1, STOCK_TARGET + 1):
    name = "BOL-G14-%03d" % i
    lot = env["stock.lot"].search([("name", "=", name), ("product_id", "=", prod.id)], limit=1)
    if not lot:
...
for day, qty in DAYS:
    ...
    print(
        f"  {origin}  {so.name}  qty={line.product_uom_qty:.0f}  "
...
# --- month end --------------------------------------------------------------
monthly = env.ref("sale_invoice_frequency.sale_invoice_frequency_monthly", raise_if_not_found=False)
...
for inv in invoices:
    ...
    for l in inv.invoice_line_ids:
        print(
            f"    {l.product_id.default_code}  qty={l.quantity:.0f}  "
            f"unit={l.price_unit:.2f}  remise={l.discount:.0f}%  "
            f"net={l.price_subtotal:.2f}"
        )
    print(
        f"  total HT: {inv.amount_untaxed:.2f}  "
